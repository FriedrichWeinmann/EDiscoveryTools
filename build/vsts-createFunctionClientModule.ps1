...
$parentModule = 'EDiscoveryTools'
if (-not $ModuleName) { $ModuleName = 'EDiscoveryTools.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\EDiscoveryTools'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\EDiscoveryTools.psd1"
$prereqHash = @{
