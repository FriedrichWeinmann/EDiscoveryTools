...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'EDiscoveryTools.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'EDiscoveryTools.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'EDiscoveryTools.Client.ProtectedToken'
